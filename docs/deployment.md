...
```
*note* [ortelius tags](https://github.com/lasthyphen/archiver/tags)
```
# make production_start
```
...
```
docker run --volume .../github.com/lasthyphen/archiver/docker/my.cnf:/etc/mysql/my.cnf --network host -e MYSQL_ROOT_PASSWORD=password -e MYSQL_DATABASE=ortelius mysql:8.0.26
```
The standard mysql defaults can cause issues, please configure the customizations [here](https://github.com/lasthyphen/archiver/blob/master/docker/my.cnf)

## run migrations -- *required* for all ortelius updates.
[dockerhub migrate](https://hub.docker.com/r/migrate/migrate)
```
docker run --volume .../github.com/lasthyphen/archiver/services/db/migrations:/migrations --network host "migrate/migrate:v4.14.1"  -path=/migrations/ -database "mysql://root:password@tcp(mysql:3306)/ortelius" up
```
Update the docker params as needed to match the user/password/host/database as appropriate.

## update ortelius configs
Update [config](https://github.com/lasthyphen/archiver/blob/master/docker/config.json) with the correct dsn in your local github repo.
...
[Configuration for Ortelius](https://github.com/lasthyphen/archiver/blob/master/docker/config.json).
